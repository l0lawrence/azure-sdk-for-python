...
from typing import Any, TYPE_CHECKING, TypeVar, Type, Awaitable, Optional, Dict
...
class CrudClient:
    ...
    async def delete(  # pylint: disable=inconsistent-return-statements
        ...
        **kwargs: Any
        ...
        _stream = False
        response = await self._send_request(request, stream=_stream, **kwargs)
    ...
    async def list(
        ...
        **kwargs: Any
    ) -> AsyncItemPaged[ResourceType[Any, PathParamsT]]:
        ...
        :rtype: AsyncItemPaged[ResourceType[Any, PathParamsT]]
        :raises ~azure.core.exceptions.HttpResponseError:
        """
        _headers = kwargs.pop("headers", {}) or {}
        _params = case_insensitive_dict(kwargs.pop("params", {}) or {})
        ...
        async def prepare_request(next_link=None):
            if not next_link:
        ...
        async def extract_data(pipeline_response):
            # Parse the response
            data = await pipeline_response.http_response.read()
            data_dict = json.loads(data.decode('utf-8'))

            # Extract the list of items and next link
            list_of_items = data_dict.get("value", [])
            ...
            next_link = data_dict.get("nextLink") or None
            return next_link, iter(deserialized_items)

        async def get_next(next_link=None):
            _request = await prepare_request(next_link)
            _stream = False
            pipeline_response = await self._client.pipeline.run(  # pylint: disable=protected-access,no-member #TODO: investigate no-member
