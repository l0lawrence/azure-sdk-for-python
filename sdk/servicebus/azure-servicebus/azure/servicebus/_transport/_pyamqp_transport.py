...
class PyamqpTransport(AmqpTransport):   # pylint: disable=too-many-public-methods
    ...
    def iter_contextual_wrapper(
        ...
        while True:
            try:
                # pylint: disable=protected-access
                receiver._original_timeout = None
                if max_wait_time:
                    receiver._original_timeout = max_wait_time
                    receiver._handler._timeout = max_wait_time
                message = receiver._inner_next(wait_time=max_wait_time)
                links = get_receive_links(message)
                with receive_trace_context_manager(receiver, links=links):
                    yield message
            except StopIteration:
                if receiver._original_timeout:
                    receiver._handler._timeout = receiver._original_timeout
                break
    ...
    def enhanced_message_received(
        ...
        if receiver._receive_context.is_set():
            ...
            print("In enhanced_message_received --release")
